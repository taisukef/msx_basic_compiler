...
// --------------------------------------------------------------------
CEXPRESSION_NODE* CEXPRESSION_OPERATOR_GE::optimization( CCOMPILE_INFO *p_info ) {
	CEXPRESSION_NODE* p;
...
void CEXPRESSION_OPERATOR_GE::compile( CCOMPILE_INFO *p_info ) {
	...
	if( compare_type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::SBC, CCONDITION::NONE, COPERAND_TYPE::REGISTER, "HL", COPERAND_TYPE::REGISTER, "DE" );	//	HL(左項) >= DE(右項) が成立する場合は S=0 (P), しない場合は S=1 (M)
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::JP, CCONDITION::M, COPERAND_TYPE::CONSTANT, s_label, COPERAND_TYPE::NONE, "" );
		p_info->assembler_list.body.push_back( asm_line );
		...
		p_info->assembler_list.body.push_back( asm_line );
		//	A= 1 → ARG < DAC : A =-1 にしたい
		//	A= 0 → ARG = DAC : A =-1 にしたい
		//	A=-1 → ARG > DAC : A = 0 にしたい
