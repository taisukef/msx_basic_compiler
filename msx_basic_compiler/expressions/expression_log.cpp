...
CEXPRESSION_NODE* CEXPRESSION_LOG::optimization( CCOMPILE_INFO *p_info ) {
	...
	if( (p_info->options.optimize_level >= COPTIMIZE_LEVEL::NODE_ONLY) && this->p_operand->is_constant ) {
		...
		if( this->p_operand->type != CEXPRESSION_TYPE::STRING ) {
			...
			p_term->set_double( log( std::stod( this->p_operand->s_value ) ) );
			return p_term;
		}
	}
	return nullptr;
...
void CEXPRESSION_LOG::compile( CCOMPILE_INFO *p_info ) {
	...
	if( this->p_operand->type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::CALL, CCONDITION::NONE, COPERAND_TYPE::CONSTANT, "bios_log", COPERAND_TYPE::NONE, "" );
		p_info->assembler_list.body.push_back( asm_line );
	}
