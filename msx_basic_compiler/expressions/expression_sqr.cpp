...
// --------------------------------------------------------------------
void CEXPRESSION_SQR::compile( CCOMPILE_INFO *p_info ) {
	...
	p_info->assembler_list.add_label( "bios_sqr", "0x02aff" );
	if( this->p_operand->type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::CALL, CCONDITION::NONE, COPERAND_TYPE::CONSTANT, "bios_sqr", COPERAND_TYPE::NONE, "" );
		p_info->assembler_list.body.push_back( asm_line );
	}
