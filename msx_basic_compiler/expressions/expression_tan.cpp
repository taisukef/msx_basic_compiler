...
// --------------------------------------------------------------------
CEXPRESSION_NODE* CEXPRESSION_TAN::optimization( CCOMPILE_INFO *p_info ) {
	CEXPRESSION_NODE *p;
...
// --------------------------------------------------------------------
void CEXPRESSION_TAN::compile( CCOMPILE_INFO *p_info ) {
	...
	p_info->assembler_list.add_label( "bios_tan", "0x029FB" );
	...
	if( this->p_operand->type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::CALL, CCONDITION::NONE, COPERAND_TYPE::CONSTANT, "bios_tan", COPERAND_TYPE::NONE, "" );
		p_info->assembler_list.body.push_back( asm_line );
	}
