...
// --------------------------------------------------------------------
CEXPRESSION_NODE* CEXPRESSION_OPERATOR_LE::optimization( CCOMPILE_INFO *p_info ) {
	CEXPRESSION_NODE* p;
...
void CEXPRESSION_OPERATOR_LE::compile( CCOMPILE_INFO *p_info ) {
	...
	if( compare_type == CEXPRESSION_TYPE::INTEGER ) {
		...
		p_info->assembler_list.body.push_back( asm_line );
		asm_line.set( CMNEMONIC_TYPE::SBC, CCONDITION::NONE, COPERAND_TYPE::REGISTER, "HL", COPERAND_TYPE::REGISTER, "DE" );	//	HL(右項) >= DE(左項) が成立する場合は S=0 (P), しない場合は S=1 (M)
		p_info->assembler_list.body.push_back( asm_line );
